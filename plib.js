/*============================================================================*\
  ...
  ** to be revised
  ...
  0.9.2   binding canvas size to parent size
/*============================================================================*\
...
present.entry=function(id,val,opt,cb){
  ...
  if(typeof pres !== 'object' || !pres.valueChange){
    ...
    if(typeof pres.tag.getContext === 'function'){
      pres.graphicOut=true;
      // Bind size to parent client size and subtract padding
      // (Canvas size is not the same as style size) 
      pres.tag.height=pres.tag.parentNode.clientHeight
        -parseFloat(window.getComputedStyle(pres.tag.parentNode).getPropertyValue('padding-top'))
        -parseFloat(window.getComputedStyle(pres.tag.parentNode).getPropertyValue('padding-bottom'));
      pres.tag.width=pres.tag.parentNode.clientWidth
        - parseFloat(window.getComputedStyle(pres.tag.parentNode).getPropertyValue('padding-left'))
        - parseFloat(window.getComputedStyle(pres.tag.parentNode).getPropertyValue('padding-right'));
    }
...
//      if(!pres.color && present_colors.indexOf(type) >=0) pres.color=type;
      if(present_colors.indexOf(pres.color) === null) pres.color="pressure";
  ...
  // Handle resize of parent
/*  
  pres.tag.parentElement.onresize = function(id,val,opt){
    console.log("resizeing",id,val,opt);
    present.entry(id,val,opt);
  }
  */
}
...
present.gauge= function(pres){
  ...
  // Set some dimentions
  if(pres.tag.clientHeight>pres.tag.clientWidth)
    var r=pres.tag.width/2.1
  else
    var r=pres.tag.height/2.1
  var rw=r/20;
  ...
  if(!pres.diskLayer || !pres.valueLayer){
    ...
    housingLayer.translate(pres.tag.clientWidth/2,pres.tag.clientHeight/2);
    pres.diskLayer.translate(pres.tag.clientWidth/2,pres.tag.clientHeight/2);
    scaleLayer.translate(pres.tag.clientWidth/2,pres.tag.clientHeight/2);
    pres.valueLayer.translate(pres.tag.clientWidth/2,pres.tag.clientHeight/2);
...
present.bar= function(pres){
  ...
  // Set some dimentions
  var fx;
  var width=pres.tag.width*0.95;
  var height=pres.tag.height*0.95;
  var text='';
  ...
  if(!pres.plateLayer || !pres.valueLayer || !pres.prefixLayer){
    ...
    pres.plateLayer.translate(0,(pres.tag.clientHeight-height)/2);
    housingLayer.translate(0,(pres.tag.clientHeight-height)/2);
    pres.prefixLayer.translate(0,(pres.tag.clientHeight-height)/2);
    pres.valueLayer.translate(mx,(pres.tag.clientHeight-height)/2+my-mym);
...
/*
  ...
  // When it gets implemented in browsers, use "DOMAttrModified" event
